...
Role.openChangeRole = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/system/role/role_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
Role.delRole = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/system/role/remove", function () {
                Feng.success("删除成功!");
                Role.table.refresh();
            }, function (data) {
...
Role.assign = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/system/role/role_assign/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
