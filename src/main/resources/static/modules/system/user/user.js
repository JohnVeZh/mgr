...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/system/user/user_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        var operation = function(){
            ...
            var ajax = new $ax(Feng.ctxPath + "/system/user/delete", function () {
                Feng.success("删除成功!");
...
$(function () {
    ...
    var table = new BSTable("managerTable", "/system/user/list", defaultColunms);
