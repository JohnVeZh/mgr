...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/system/log/detail/" + this.seItem.id, function (data) {
...
OptLog.delLog = function () {
    Feng.confirm("是否清空所有日志?",function(){
        var ajax = Feng.baseAjax("/system/log/delLog","清空日志");
        ajax.start();
...
OptLog.formParams = function() {
    ...
    queryData['logntype'] = $("#logType").val();
...
$(function () {
    ...
    var table = new BSTable(OptLog.id, "/system/log/list", defaultColunms);
