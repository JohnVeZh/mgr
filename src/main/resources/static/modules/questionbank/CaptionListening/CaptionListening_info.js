...
CaptionListeningInfoDlg.close = function() {
    parent.layer.close(window.parent.CaptionListening.layerIndex);
}
...
CaptionListeningInfoDlg.collectData = function() {
    this.set('id').set('type').set('paperId').set('name')
        .set('sort').set("audioUrl").set('subtitleUrl');
}
...
CaptionListeningInfoDlg.addSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/CaptionListening/add", function(data){
        Feng.success("添加成功!");
        window.parent.CaptionListening.table.refresh();
...
CaptionListeningInfoDlg.editSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/CaptionListening/update", function(data){
        Feng.success("修改成功!");
        window.parent.CaptionListening.table.refresh();
        CaptionListeningInfoDlg.close();
    },function(data){
