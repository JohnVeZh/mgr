    ...
    _e(function (E, $) {
        ...
        E.createMenu(function (check) {
            ...
            var menu = new E.Menu({
                editor: editor,
                id: menuId,
                title: lang.sparkLink
            });

            // 创建 dropPanel
            var $content = $('<div></div>');
            var $divText = $('<div style="margin:20px 10px;" class="clearfix"></div>');
            var $divLink = $divText.clone();
            var $divSubmit = $divText.clone().css('margin', '0 10px');
            var $divTypeSelect = $divText.clone();
            var $divIdList = $divText.clone();
            var $textInput = $('<input type="text" class="block" placeholder="' + lang.text + '"/>');
            var $urlInput = $('<input type="text" class="block" placeholder="' + lang.sparkLink + '"/>');
            var $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
            var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
            //选择跳转类型
            var select = '<select id="sparkLinkType" name="类型" >';
            select += '<option value="1">外部链接</option>';
            select += '<option value="2">活动</option>';
            select += '<option value="3">资讯</option>';
            select += '</select>';
            var $typeSelect = $(select);
            //选择具体内容的id
            var ids = '<select id="sparkLinkContentId" name="类型" style="width: 250px;">';
            ids += '</select>';
            var $idsSelect = $(ids);;
            ...
            $typeSelect.change(function(e){
                ...
                if (typeValue==1){
                    $divLink.show();
                    $divIdList.hide();
                    if($urlInput.val().indexOf('http')<0){
                        $urlInput.val('http://');
                    }
                    ...
                    $divLink.hide();
                    //查询活动
                    var ajax = new $ax(Feng.ctxPath + "/community/activity/list/without/content", function (data) {
                        //更新选项
                        var sparkLinkContentIdOpt = "";
                        for (i in data.rows){
                            if(data.rows[i].id){
                                sparkLinkContentIdOpt +=( "<option value='"+data.rows[i].id+"' >"+data.rows[i].title+"</option>");
                            }
                        }

                        $idsSelect.html(sparkLinkContentIdOpt);

                    }, function (data) {
                        Feng.error("获得活动失败,请刷新页面!" + data.responseJSON.message + "!");
                    });
                    ajax.start();
                }else if (typeValue==3){
                    $divIdList.show();
                    $divLink.hide();
                    //查询资讯
                    var ajax = new $ax(Feng.ctxPath + "/community/news/list/without/content", function (data) {
                        //更新选项
                        var sparkLinkContentIdOpt = "";
                        for (i in data.rows){
                            if(data.rows[i].id){
                        ...
                        Feng.error("获得资讯失败,请刷新页面!" + data.responseJSON.message + "!");
                    });
                    ajax.start();
            ...
            $divTypeSelect.append($typeSelect);
            $divText.append($textInput);
            $divLink.append($urlInput);
            $divIdList.append($idsSelect);
            $divSubmit.append($btnSubmit).append($btnCancel);
            $content.append($divTypeSelect).append($divText).append($divLink).append($divIdList).append($divSubmit);
            ...
            menu.clickEvent = function (e) {
                ...
                var contentId = "";
                if (rangeElem ) {
                    if(rangeElem.attributes.type){
                        type = rangeElem.attributes.type.value;
                    }

                    if(type == 1){
                        url = rangeElem.href || '';
                    }else if(type!=1 && rangeElem.attributes.content){
                        contentId = rangeElem.attributes.content.value;
                    }
                }
                ...
                text && $textInput.val(text);
                $typeSelect.val(type);
                contentId && $idsSelect.val(contentId);
            ...
            $btnSubmit.click(function (e) {
                ...
                var $txt = editor.txt.$txt;
                var $oldLinks, $newLinks;
                var uniqId = 'sparkLink' + E.random();

                // 获取数据
                var url = $.trim($urlInput.val());
                var text = $.trim($textInput.val());
                var type = $.trim($typeSelect.val());
                var contentId = $.trim($idsSelect.val());
                ...
                if (!isRangeEmpty) {
                    ...
                    $newLinks.attr('target', '_blank'); // 增加 _blank
                    if(type!=1){
                        $newLinks.attr('onclick', 'spark.contentJump("{\\"type\\":\\"'+type+'\\",\\"content\\":\\"'+contentId+'\\" }")');
                        $newLinks.attr('type', type);
                        $newLinks.attr('content', contentId);
                    }
                    ...
                    commandFn = function () {
                        if(type==1){
                            ...
                            $linkElem.text(text);
                            $linkElem.attr('onclick', 'spark.contentJump("{\\"type\\":\\"'+type+'\\",\\"content\\":\\"'+contentId+'\\" }")');
                            $linkElem.attr('type', type);
                            $linkElem.attr('content', contentId);
                        }
                    ...
                    if(type==1){
                        ...
                        //内部链接
                        linkHtml = "<a href=\"javascript:void(0);\" type=\""+type+"\" content=\""+contentId+"\" onclick=\"spark.contentJump('{\\'type\\':\\'"+type+"\\',\\'content\\':\\'"+contentId+"\\'}')\">" + text + "</a>";
                    }
